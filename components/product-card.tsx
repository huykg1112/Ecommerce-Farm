  ...
  return (
    <Card className="overflow-hidden product-card border-none shadow-md">
      <div className="relative">
        ...
        {product.featured && <Badge className="absolute bottom-2 left-2 bg-primary">Nổi bật</Badge>}
      </div>
      <CardContent className="p-4">
        <div className="text-sm text-gray-500 mb-1">{product.category}</div>
        <Link href={`/products/${product.id}`} className="hover:underline">
          <h3 className="font-semibold text-lg line-clamp-2 h-12">{product.name}</h3>
        </Link>
        ...
        <div className="flex items-center mt-2">
          <Link href={`/seller/${product.seller.id}`} className="text-sm text-primary hover:underline">
            {product.seller.name}
          </Link>
        </div>
      </CardContent>
      <CardFooter className="p-4 pt-0 flex items-center justify-between">
        <div className="flex items-center">
          ...
          {product.originalPrice && (
            <span className="text-gray-400 line-through text-sm ml-2">{product.originalPrice.toLocaleString()}đ</span>
          )}
        </div>
        <Button size="sm" onClick={handleAddToCart} className="bg-primary hover:bg-primary-dark">
