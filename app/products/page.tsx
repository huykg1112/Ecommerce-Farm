...
type SortOption = "featured" | "newest" | "price-low-high" | "price-high-low" | "best-selling" | "rating"
...
export default function ProductsPage() {
  ...
  const sellers = Array.from(new Set(products.map((product) => product.seller.name)))
  ...
  const filteredProducts = products.filter((product) => {
    ...
    // Lọc theo đại lý
    if (filters.sellers.length > 0 && !filters.sellers.includes(product.seller.name)) {
      return false
    }
  ...
  const renderActiveFilters = () => {
    ...
    filters.categories.forEach((category) => {
      activeFilters.push(
        <Badge key={`category-${category}`} variant="outline" className="flex items-center gap-1 m-1">
          {category}
    ...
    if (filters.rating) {
      activeFilters.push(
        <Badge key="rating" variant="outline" className="flex items-center gap-1 m-1">
          {`${filters.rating}+ sao`}
    ...
    filters.sellers.forEach((seller) => {
      activeFilters.push(
        <Badge key={`seller-${seller}`} variant="outline" className="flex items-center gap-1 m-1">
          {seller}
    ...
    if (filters.onSale) {
      activeFilters.push(
        <Badge key="on-sale" variant="outline" className="flex items-center gap-1 m-1">
          Đang giảm giá
    ...
    if (searchTerm) {
      activeFilters.push(
        <Badge key="search" variant="outline" className="flex items-center gap-1 m-1">
          Tìm: {searchTerm}
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="hidden md:block w-64 shrink-0">
          <div className="sticky top-24">
            ...
            <Accordion type="multiple" defaultValue={["categories", "price", "rating", "sellers", "other"]}>
                    ...
                    {[5, 4, 3, 2, 1].map((rating) => (
                      <div key={rating} className="flex items-center space-x-2">
                        <Checkbox
                          ...
                          onCheckedChange={() => handleRatingChange(filters.rating === rating ? null : rating)}
                        />
        ...
        <div className="flex-1">
          ...
          {activeFilterCount > 0 && (
            <div className="hidden md:flex flex-wrap items-center mb-4">
              <span className="text-sm font-medium mr-2">Bộ lọc đang áp dụng:</span>
              {renderActiveFilters()}
            </div>
          )}
